...
class MediaService:
    ...
    def is_text_file(filename: str, mime_type: str) -> bool:
        ...
        """
        text_extensions = {
            '.py', '.txt', '.json', '.xml', '.yaml', '.yml', '.ini', '.cfg', '.conf',
            '.md', '.csv', '.sh', '.bat', '.js', '.ts', '.css', '.html', '.htm',
            '.sql', '.rs', '.go', '.c', '.cpp', '.h', '.java', '.kt', '.swift', '.rb'
        }
        ext = os.path.splitext(filename.lower())[1]
        if ext in text_extensions:
            return True
        if mime_type and (mime_type.startswith('text/') or mime_type in ['application/json', 'application/xml', 'application/x-javascript']):
            return True
        return False

    @classmethod
    async def process_message_media(cls, message: Message, override_text: str = None) -> tuple[str, list[dict]]:
        """
        ...
        """
        text = override_text if override_text is not None else (message.text or message.caption or "")
        media_parts = []
